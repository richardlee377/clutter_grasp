...
def ConvexifyandSolve(x,y,xdes,ydes,Wk,A):
    fx_grad = Bk*np.matrix(x) + 2*C2*mfng*np.abs(np.matrix(x) - np.matrix(xdes))
    fy_grad = Bk*np.matrix(y) + 2*C2*mfng*np.abs(np.matrix(y) - np.matrix(ydes))
    ...
    hx  = matrix(np.concatenate((v_max+vx,v_max+vx), axis=0))
    hy  = matrix(np.concatenate((v_max+vy,v_max+vy), axis=0))

    atmp = np.zeros((1,N-1))
    a  = matrix(np.concatenate((np.matrix(1),atmp),axis = 1))
    b  = matrix(np.zeros(1)) 
    print b
...
xfng, yfng, xdes, ydes, fx, fy = simulate("naive",0)
...
# Wk = Bk
c   = 0.01 # fraction of TrueImprove/ModelImprove
k   = 1.2 # penalty scaling factor
tau_plus  = 1.10 # trust region expansion
tau_minus = 0.50 # trust region shrinkage
f_tol     = 1000  # acceptable cost from cost function
x_tol     = 0.1  # smallest trust region size
