...
def KE(body):
    ...
    KEx = 0.5*body.mass*(vl[0]/TARGET_FPS)**2  + 0.5*body.inertia*np.linalg.norm(va/TARGET_FPS)**2
    KEy = 0.5*body.mass*(vl[1]/TARGET_FPS)**2  + 0.5*body.inertia*np.linalg.norm(va/TARGET_FPS)**2
    return KEx, KEy
...
def simulate(cmd, trj):
    ...
    obs1_body = world.CreateDynamicBody(position=(17, 9), angle=30,
...
def ConvexifyandSolve(x,y,xdes,ydes,Wk,A):
    ...
    # solx = solvers.qp(Q, px, G, hx)
    # soly = solvers.qp(Q, px, G, hy)
...
def plot_pos(x0, y0, xf, yf):
    # import matplotlib.pyplot as plt
    t = np.transpose(np.matrix(range(0, N)))
    plt.figure(1)
    plt.subplot(211)
...
f_tol     = 5000  # acceptable cost from cost function
